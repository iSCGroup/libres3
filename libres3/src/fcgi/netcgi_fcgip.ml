(* Based on OCamlnet-3.6 Netcgi_fcgi.ml with a minor change:
 * allow for missing Content-Type for PUT/POST when Content-Length is 0.
 * This file is copied to avoid having to patch&build the entire ocamlnet code
 * *)
(* LICENSE:
  *
...
class fcgi_environment_p ?log_error ~config ~properties ~input_header fcgi_ch id
  ...
  let split_query_re = Netstring_str.regexp "[&;]" in
object
  inherit cgi_environment ~config ~properties ~input_header user_ch as super

  (* Netencoding.Url.dest_url_encoded_parameters doesn't support empty values
   * or names, so "fixup" the query string to accept URLs like:
   * GET /?&max-keys=0 *)

  method cgi_query_string =
    let query = super#cgi_query_string in
    let params = Netstring_str.split split_query_re query in
    let fixed_params = List.rev_map (fun p ->
      let name, value =
      begin try
        let pos = String.index p '=' in
        let next = pos + 1 in
        String.sub p 0 pos,
        if next = String.length p then ""
        else String.sub p next ((String.length p) - next)
      with Not_found ->
        p, ""
      end in
      name ^ "=" ^ value
    ) params in
    String.concat "&" fixed_params

  method input_content_type () =
    try
      super#input_content_type ()
