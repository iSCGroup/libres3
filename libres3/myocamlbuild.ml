(* OASIS_START *)
(* DO NOT EDIT (digest: 9e873bed4e52d6454ab148e5c0bc0f07) *)
...
let packages = [
  "netclient";"netstring";"threads";"jsonm";"cryptokit";"lwt";
  "xmlm";"netsys";"str";"netcgi2";"ocsigenserver"
]
...
let package_info () =
  String.concat "\n" (List.map (fun package ->
    try
      Ocamlbuild_pack.My_unix.run_and_read
        ("ocamlfind query -r -long-format " ^ package)
    with e -> Printexc.to_string e
  ) packages)

let env_info () =
  let vars =
    ["OCAMLFIND_CONF";"OCAMLPATH";"OCAMLFIND_DESTDIR";"OCAMLFIND_METADIR";"OCAMLFIND_COMMANDS";
     "CAMLLIB";"OCAMLLIB";"OCAMLFIND_LDCONF";"OCAMLFIND_IGNORE_DUPS_IN";
     "OCAMLBUILD";"OCAMLBUILD_FIND";
     "CAML_LD_LIBRARY_PATH";"OCAML_TOPLEVEL_PATH";
     "CPPFLAGS";"LDFLAGS"
    ] in
  String.concat "" (List.map (fun env ->
    try Printf.sprintf "%s=%s\n" env (Sys.getenv env) with Not_found -> ""
  ) vars)

let make_version _ _ =
  let ver_entry = Printf.sprintf "let version = %S;;\n" (version ()) in
  let package_entry =
    Printf.sprintf "let package_info = %S;;\n" (package_info ()) in
  let env_entry =
    Printf.sprintf "let env_info = %S;;\n" (env_info ())
  in
  Echo ([ver_entry; package_entry; env_entry] , "version.ml")
;;
...
(function
  | After_rules as e ->
      ...
      flag ["ocaml"; "link";  "link_ocsigen"] & S[
        A"-predicates";A"nocommandline"
      ];
      dispatch_default e;
  | e->
      dispatch_default e
